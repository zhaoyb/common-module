...
public class RandomLoadBalance extends AbstractLoadBalance {
    ...
    protected IpPort doSelect(final List<IpPort> ipPorts, final String key) {
        // 总个数
        int length = ipPorts.size();
        // 总权重
        int totalWeight = 0;
        // 权重是否都一样
        boolean sameWeight = true;
        for (int i = 0; i < length; i++) {
            int weight = ipPorts.get(i).getWeight();
            // 累计总权重
            totalWeight += weight;
            if (sameWeight && i > 0
                    && weight != ipPorts.get(i - 1).getWeight()) {
                // 计算所有权重是否一样
                sameWeight = false;
            }
        }
        ...
        // 如果权重相同或权重为0则均等随机
        return ipPorts.get(RANDOM.nextInt(length));
    }
}
